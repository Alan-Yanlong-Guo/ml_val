...
def train_test_process(data_start=1975, tr_start=1975, tr_duration=20, ts_duration=1, dy=0, dq=1, plot=False):
    ...
    for item in y_tr.columns:
        if item not in variables_to_test:
            continue
        param_test = {'max_depth': [1, 2, 6], 'num_leaves': [2, 8], 'n_estimators': [20, 100, 200]}
...
for tr_start in range(start_training_year, end_training_year, 1):
    ...
    oosr2, max1, max2, max3, max4, max5, y_trues, y_preds = train_test_process(data_start=start_training_year, tr_start=tr_start, tr_duration=20, ts_duration=1, dy=0, dq=1)
    oosr2_cum = {}
    ...
    if oosr2df is None:
        ...
        oosr2df = oosr2df.drop(columns=['avg'])
